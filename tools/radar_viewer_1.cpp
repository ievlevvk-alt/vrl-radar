...
public:
    RadarViewer(int width = 1280, int height = 960) 
        ...
        std::cout << "[DEBUG] Constructor: max_range_km=" << max_range_km_ 
                  << " current_range_km=" << current_range_km_ << std::endl;
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (event.key.keysym.sym == SDLK_ESCAPE) {
                    ...
                    std::cout << "[DEBUG] Reset zoom: current_range_km was " << current_range_km_;
                    current_range_km_ = max_range_km_;
                    std::cout << ", now " << current_range_km_ << std::endl;
                    update_scale();
                }
...
private:
    ...
    void update_scale() {
        ...
        // Ограничиваем текущую дальность
        if (current_range_km_ < min_range_km_ - 0.001) {
            std::cout << "[DEBUG] Clamping current_range_km from " << current_range_km_ 
                      << " to " << min_range_km_ << std::endl;
            current_range_km_ = min_range_km_;
        }
        if (current_range_km_ > max_range_km_ + 0.001) {
            std::cout << "[DEBUG] Clamping current_range_km from " << current_range_km_ 
                      << " to " << max_range_km_ << std::endl;
            current_range_km_ = max_range_km_;
        }
        ...
        std::cout << "[DEBUG] update_scale: current_range_km=" << std::fixed << std::setprecision(4)
                  << current_range_km_ << " scale=" << scale_ << std::endl;
    }
    
    void zoom_in() {
        std::cout << "[DEBUG] zoom_in: current_range_km was " << std::fixed << std::setprecision(4) << current_range_km_;
        double new_range = current_range_km_ * 0.8;
        if (new_range >= min_range_km_ - 0.001) {
            current_range_km_ = new_range;
            std::cout << ", now " << current_range_km_ << std::endl;
            update_scale();
        } else {
            std::cout << ", cannot zoom more (min=" << min_range_km_ << ")" << std::endl;
        }
    }
    
    void zoom_out() {
        std::cout << "[DEBUG] zoom_out: current_range_km was " << std::fixed << std::setprecision(4) << current_range_km_;
        double new_range = current_range_km_ * 1.25;
        if (new_range <= max_range_km_ + 0.001) {
            current_range_km_ = new_range;
            std::cout << ", now " << current_range_km_ << std::endl;
            update_scale();
        } else {
            std::cout << ", cannot zoom out more (max=" << max_range_km_ << ")" << std::endl;
        }
    }
    ...
    void draw_grid() {
        ...
        // Дальномерные круги от 50 до 200 км с шагом 50 км
        ...
        // Азимутальные метки - рисуем от дальности 10 км
        SDL_SetRenderDrawColor(renderer_, 50, 50, 50, 255);
        double start_range_m = 10000.0;  // 10 км в метрах
        int start_radius = static_cast<int>(start_range_m * scale_);
        
        // Рисуем метки только если начальный радиус виден
        if (start_radius < std::min(width_, height_) / 2) {
            for (int az = 0; az < 360; az += 30) {
                ...
                // Подписи направлений на внешнем круге
    ...
    void draw_info() {
        ...
        snprintf(info, sizeof(info), 
                 ...
                 current_scan_.rbs_replies.size(), current_scan_.uvd_replies.size(),
        ...
        const char* help = "Mouse wheel: Zoom in/out | R: Reset zoom (200 km) | ESC: Exit";
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  R: Reset zoom (200 km)\n";
    std::cout << "  ESC: Exit\n\n";
    
    auto config = SystemConfig::load_from_file("radar.conf");
    config.tracker.debug_mode = false;
    
    config.radar.range_bin_rbs = 75.0;
    config.radar.range_bin_uvd = 100.0;
